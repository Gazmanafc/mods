 ...
 */
 ...
 // Edited a bit by Daniel15 for the Akismet SMF mod
...
class Akismet
{
	...
	function Akismet($blogURL, $wordPressAPIKey)
	{
		...
		if ($response === false)
		{
			// TODO: Handle connection errors. For now, it's only handled in the isSpam() function
		}
		elseif ($response[1] != 'valid')
		{
			// Whoops, no it's not.  Throw an exception as we can't proceed without a valid API key.
			fatal_lang_error('akismet_invalid_api');
		}
	}
	...
	function http_post($request, $host, $path)
	{
		$http_request  = 
				...
				"User-Agent: Akismet PHP5 Class " . $this->version . " (modified by Daniel15) | Akismet/1.11\r\n" .
				"\r\n" .
				$request
			;
...
class SocketWriteRead
{
	...
	function send()
	{
		...
		if($this->errorNumber != 0 || $fs === false)
		{
			...
			// Log the error to the SMF error log
			log_error(sprintf($txt['akismet_cant_connect'], $this->host, $this->errorNumber, $this->errorString));
			// Return false
			return false;
